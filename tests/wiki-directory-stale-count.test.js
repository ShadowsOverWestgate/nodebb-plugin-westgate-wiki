...
(async () => {
  ...
  assert.equal((await wikiPaths.resolveWikiNode("Wiki/Server_Rules", { uid: 1 })).status, "ok");
  assert.equal(
    (await wikiPaths.resolveWikiNode("Wiki/Third_Party_Content_Credits", { uid: 1 })).status,
    "ok",
