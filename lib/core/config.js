...
function normalizeSettings(settings = {}) {
  ...
  const homeTopicId = normalizePositiveInt(settings.homeTopicId);
  ...
  const routeRootCid = normalizePositiveInt(settings.routeRootCid);
...
function isLegacyWikiRouteRootCategory(category, configuredCids) {
  const cid = normalizePositiveInt(category && category.cid);
...
async function ensureSingleLegacyRouteRootCid() {
  ...
  await meta.settings.set(SETTINGS_KEY, {
    ...current,
    routeRootCid: String(normalizePositiveInt(routeRootCandidates[0].cid))
  });
...
async function setHomeTopicIdInSettings(tid) {
  const n = normalizePositiveInt(tid);
  if (!n) {
