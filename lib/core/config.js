...
function normalizeRouteRootCid(rawValue) {
  if (rawValue === undefined || rawValue === null || rawValue === "") {
    return null;
  }
...
function normalizeSettings(settings = {}) {
  ...
  const routeRootCid = normalizeRouteRootCid(settings.routeRootCid);
  const routeRootCidText = settings.routeRootCid === undefined || settings.routeRootCid === null || settings.routeRootCid === ""
    ? ""
    : String(settings.routeRootCid).trim();
  const canonicalPathMigrationVersion = settings.canonicalPathMigrationVersion === undefined || settings.canonicalPathMigrationVersion === null
    ? ""
    : String(settings.canonicalPathMigrationVersion).trim();
...
function isLegacyWikiRouteRootCategory(category, configuredCids) {
  const cid = normalizeRouteRootCid(category && category.cid);
  if (!cid || !configuredCids.has(cid)) {
    return false;
  }

  return wikiSlug.getSlugLeaf(category && category.slug).toLowerCase() === "wiki";
}

async function ensureSingleLegacyRouteRootCid() {
  const current = await meta.settings.get(SETTINGS_KEY) || {};
  const settings = normalizeSettings(current);
  if (settings.routeRootCid || !settings.categoryIds.length || typeof categories.getCategoryData !== "function") {
    return;
  }

  const configuredCids = new Set(settings.categoryIds);
  const rows = (await Promise.all(settings.categoryIds.map((cid) => categories.getCategoryData(cid)))).filter(Boolean);
  const routeRootCandidates = rows.filter((category) => isLegacyWikiRouteRootCategory(category, configuredCids));
  if (routeRootCandidates.length !== 1) {
    return;
  }

  await meta.settings.set(SETTINGS_KEY, {
    ...current,
    routeRootCid: String(normalizeRouteRootCid(routeRootCandidates[0].cid))
  });
...
async function ensureDefaults() {
  ...
  await ensureSingleLegacyRouteRootCid();
}

// Plain clear of THIS module's cache only. Called by wiki-cache-invalidation;
...
function invalidateSettingsCache() {
  try {
    require("./wiki-cache-invalidation").invalidateAll();
  } catch (e) {
...
async function loadSettings() {
  ...
  // Only publish if no invalidation raced our awaits; otherwise we'd revive a
  // stale snapshot and serve it for the whole TTL. Caller still gets `loaded`.
...
async function setCanonicalPathMigrationVersion(version) {
  const normalizedVersion = String(version || "").trim();
  if (!normalizedVersion) {
    throw new Error("invalid-canonical-path-migration-version");
  }
  const current = await meta.settings.get(SETTINGS_KEY) || {};
  await meta.settings.set(SETTINGS_KEY, {
    ...current,
    canonicalPathMigrationVersion: normalizedVersion
  });
  invalidateSettingsCache();
}

async function setRouteRootCidInSettings(cid) {
  const n = normalizeRouteRootCid(cid);
  if (!n) {
    throw new Error("invalid-route-root-cid");
  }
  const current = await meta.settings.get(SETTINGS_KEY) || {};
  await meta.settings.set(SETTINGS_KEY, {
    ...current,
    routeRootCid: String(n)
  });
  invalidateSettingsCache();
}
...
module.exports = {
  ...
  setCanonicalPathMigrationVersion,
  setRouteRootCidInSettings,
  normalizeSettings,
  normalizeHomeTopicId,
  normalizeRouteRootCid,
