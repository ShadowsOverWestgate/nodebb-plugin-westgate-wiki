...
/**
 * Escape a raw (titleRaw-derived) string for direct Benchpress interpolation.
 * Benchpress does not escape {value}; core-escaped topic.title is safe, but
 * titlePath segments come from titleRaw and must be escaped at render time.
 * Same character set as validator.escape (minus "/", which display keeps).
 */
...
function serializeSectionLink(category) {
  return {
    ...
    wikiPath: category.wikiPath || "",
    hasWikiPath: !!category.wikiPath,
...
function serializeTopicSummary(topic) {
  // titlePath segments come from titleRaw (unescaped); Benchpress does not escape
  // {value}, so every title-derived field emitted to a template is escaped here.
  const titlePath = getTitlePath(topic.titleRaw || topic.title).map(escapeTitleHTML);
  ...
  return {
    ...
    wikiPath: topic.wikiPath || "",
    hasWikiPath: !!topic.wikiPath,
