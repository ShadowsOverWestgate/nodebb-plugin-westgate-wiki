...
function getCanonicalTitleAncestorText(nodeResult, ancestorIndex) {
  const node = nodeResult && nodeResult.node || {};
  ...
  const titleIndex = ancestorIndex - namespaceSegmentCount;
...
function forCanonicalNodeView(nodeResult, options = {}) {
  ...
  const trail = ancestors.map((ancestor, index) => ({
    text: getCanonicalTitleAncestorText(nodeResult, index) ||
      ancestor.segment ||
