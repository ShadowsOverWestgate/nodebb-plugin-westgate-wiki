...
function hasUnsafeCssSignal(css) {
  return /(?:expression|javascript|vbscript|data:text\/html|behavior\s*:|@import|url\s*\(|^|\})\s*(?:html|body|:root|\*|#content|\.container|\.navbar|\.footer|\.westgate-wiki)\b/i.test(css);
}
...
function validatePlannedPlacements(destination, namespaceOperations, pageOperations) {
  ...
  plannedNamespaces.forEach((namespace) => {
    ...
    const crossFacetMatches = pageRows.filter((row) => row.foldedKey === namespace.foldedKey);
  ...
  plannedPages.forEach((page) => {
    ...
    const crossFacetMatches = namespaceRows.filter((row) => row.foldedKey === page.foldedKey);
    if (crossFacetMatches.length) {
