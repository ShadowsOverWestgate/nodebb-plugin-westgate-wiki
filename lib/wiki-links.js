...
async function resolveExistingTopicByTitleOrSlug(cid, pageTitleOrSlug, context) {
  ...
  const resolvedMatches = exactTitleMatches.length ? exactTitleMatches : (
...
async function getArticlePathForTopic(topic, fallbackCategory, context) {
  ...
  if (context && topicWithCid && topicWithCid.cid != null) {
    ...
    if (namespacePath) {
      const rows = await context.getTopicRows(topicWithCid.cid);
      return wikiPaths.getTopicWikiPath(topicWithCid, namespacePath, wikiPaths.getTopicSlugLeafCounts(rows));
    }
  }
