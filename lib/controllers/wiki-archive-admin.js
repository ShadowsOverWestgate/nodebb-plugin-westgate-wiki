...
const archiveJobs = require("../archive/wiki-archive-jobs");
const archiveExport = require("../archive/wiki-archive-export");
const archiveImport = require("../archive/wiki-archive-import");
const archiveApply = require("../archive/wiki-archive-apply");
const archiveZip = require("../archive/wiki-archive-zip");
const archiveRuntime = require("../archive/wiki-archive-runtime");
const archiveConfig = require("../archive/wiki-archive-config");
...
// Serialize archive applies so a second request's preflight runs after the
// first's writes land and sees the collisions (otherwise both read the
// destination before either mutates and both create duplicate categories/topics).
// ponytail: single in-process lock; applies are rare and admin-only. A clustered
// deployment would need a db-backed lock to cover cross-worker double-submits.
  ...
  async function applyImportJob(req, res) {
    ...
    try {
      ...
      const result = await withApplyLock(async () => {
        // Re-read destination and re-run preflight INSIDE the lock: a serialized
        // second apply of the same archive now sees the first's categories/topics
        // and blocks instead of duplicating them.
        ...
        return { completedApplyJob, applyReport };
      });

      return apiResponse(200, res, sanitizePrivatePaths({
        ...result.completedApplyJob,
        previewJobId: job.jobId,
        applyReport: result.applyReport
      }));
    } catch (err) {
