...
function invalidateNamespace(cid) {
  if (wikiPaths && typeof wikiPaths.invalidateWikiTreeIndex === "function") {
    wikiPaths.invalidateWikiTreeIndex({ reason: "wiki-directory-invalidated" });
  }
...
async function attachCanonicalWikiPaths(rows, namespaceInfo) {
  return Promise.all((Array.isArray(rows) ? rows : []).map(async (row) => {
    const pageInfo = await wikiCanonicalPathAdapter.getCanonicalPageInfo(row, { namespaceInfo });
...
async function getDirectChildNamespaceCanonicalPaths(parsedCid, uid, settings) {
  ...
  await Promise.all(fullRows.filter(Boolean).map(async (child) => {
    const namespaceInfo = await wikiCanonicalPathAdapter.getCanonicalNamespaceInfo(child, { settings, uid });
...
/**
 * Every tid in the category, pinned set included — NodeBB removes pinned
 * topics (and scheduled topics, which it auto-pins) from cid:{cid}:tids and
 * keeps them in cid:{cid}:tids:pinned.
 */
async function getAllCategoryTids(parsedCid) {
  const [pinnedTids, tids] = await Promise.all([
    db.getSortedSetRange(`cid:${parsedCid}:tids:pinned`, 0, -1),
    db.getSortedSetRange(`cid:${parsedCid}:tids`, 0, -1)
  ]);
  return [...new Set([
    ...(Array.isArray(pinnedTids) ? pinnedTids : []),
    ...(Array.isArray(tids) ? tids : [])
  ])];
}
...
async function resolveTopicBySlugLeafForViewer(parsedCid, uid, pageSlug) {
  ...
  const matches = summaries.filter((s) => wikiPaths.getTopicSlugLeaf(s) === pageSlug);
...
async function getDirectoryWindow(cid, uid, options = {}) {
  ...
  const namespaceInfo = cat ? await wikiCanonicalPathAdapter.getCanonicalNamespaceInfo(cat, { settings: gate.settings, uid }) : null;
