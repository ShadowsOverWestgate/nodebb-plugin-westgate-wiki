...
function forArticleView(wikiPage) {
  ...
  // titlePath/parentPages texts derive from titleRaw — escape them here;
  // category names and topic.title arrive core-escaped already
  const parentCrumbs = (wikiPage.parentPages || []).map((p) => ({
    text: serializer.escapeTitleHTML(p.text),
    url: p.url || undefined
  }));
  const leafTitle = wikiPage.pageTitlePath && wikiPage.pageTitlePath.length
    ? serializer.escapeTitleHTML(wikiPage.pageTitlePath[wikiPage.pageTitlePath.length - 1])
...
function getCanonicalNodeLeafText(nodeResult, fallbackText) {
  ...
  if (node.namespace && node.namespace.category && node.namespace.category.name) {
    return serializer.escapeTitleHTML(node.namespace.category.name);
  }
...
function getCanonicalTitleAncestorText(nodeResult, ancestor) {
  ...
  return serializer.escapeTitleHTML(titlePath[titleIndex] || "");
}
...
function forComposeEdit(section, topic) {
  const title = topic.titleRaw ? serializer.escapeTitleHTML(topic.titleRaw) : topic.title;
  const trail = [
    ...mapAncestors(section.ancestorSections),
    { text: section.name, url: section.wikiPath },
