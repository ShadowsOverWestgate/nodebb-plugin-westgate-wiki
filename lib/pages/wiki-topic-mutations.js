...
const TOPIC_MUTATION_LOCK_RECOVERY_TTL_MS = 30 * 1000;
...
async function setRecoveryLockExpiry(db, lockKey) {
  if (typeof db.pexpire === "function") {
    await db.pexpire(lockKey, TOPIC_MUTATION_LOCK_RECOVERY_TTL_MS);
  } else if (typeof db.expire === "function") {
    await db.expire(lockKey, Math.ceil(TOPIC_MUTATION_LOCK_RECOVERY_TTL_MS / 1000));
  }
}
...
async function getLockFields(db, lockKey) {
...
function lockFieldsAreLive(lockFields) {
  return !!(
    Number.isFinite(lockFields.expiresAt) &&
    lockFields.expiresAt > Date.now()
  );
}

async function repairRecoverableLock(db, lockKey) {
  const lockFields = await getLockFields(db, lockKey);
  if (lockFieldsAreLive(lockFields)) {
    return false;
  }
  await db.setObjectField(
    lockKey,
    TOPIC_MUTATION_LOCK_EXPIRES_AT_FIELD,
    Date.now() + TOPIC_MUTATION_LOCK_RECOVERY_TTL_MS
  );
  await setRecoveryLockExpiry(db, lockKey);
  return true;
}
...
async function acquireTopicMutationLock(key) {
  ...
  if (
    ...
    (typeof db.pexpire !== "function" && typeof db.expire !== "function") ||
  ...
  if (count !== 1) {
    const lockFields = await getLockFields(db, lockKey);
    if (!lockFieldsAreLive(lockFields)) {
      await repairRecoverableLock(db, lockKey);
    }
  ...
  return async () => {
    ...
    if (currentOwner === ownerToken && Number.isFinite(expiresAt) && expiresAt > Date.now()) {
      await deleteLockKey(db, lockKey);
    }
  };
}
