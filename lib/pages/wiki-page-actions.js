    ...
    return await wikiTopicMutations.withTopicMutationGuard(tid, async () => {
      ...
      try {
        if (payload.cid !== currentCid) {
          await wikiTopicMutations.withManagedMutationContext(() => topics.tools.move(
            tid,
            wikiTopicMutations.markManagedMutation({ cid: payload.cid, uid: req.uid })
          ));
          moved = true;
        }
        ...
        if (changed) {
          try {
            ...
            if (moved) {
              await wikiTopicMutations.withManagedMutationContext(() => topics.tools.move(
                tid,
                wikiTopicMutations.markManagedMutation({ cid: currentCid, uid: req.uid })
              ));
            }
            wikiDirectory.invalidateNamespace(currentCid);
            wikiDirectory.invalidateNamespace(payload.cid);
  ...
  try {
    return await wikiTopicMutations.withTopicMutationGuard(tid, async () => {
      ...
      try {
        ...
        updatedTopic = await topics.getTopicData(tid);
        pathInfo = await getCanonicalWikiPathInfo(wikiPaths, updatedTopic, req.uid);
        revision = await wikiRevisions.appendRevision({
          ...revisionPayload,
          canonicalPath: pathInfo.canonicalPath,
          wikiPath: pathInfo.wikiPath
        });
...
// action:category.update hook body. A namespace's index page lives in the
// namespace's PARENT category (page path == namespace path). When the category
// is moved in the ACP, the index page would be orphaned in the old parent and
// the stored westgateWikiNamespaceIndexCid linkage would stop validating — so
// move the index page along with its namespace.
async function relocateNamespaceIndexPageOnCategoryMove(data) {
  const cid = asPositiveInt(data && data.cid);
  const modified = data && data.modified;
  if (!cid || !modified || modified.parentCid === undefined) {
    return;
  }
  ...
  try {
    ...
    // Category tree just changed; do not trust the 30s settings cache.
    const settings = await config.getSettings({ bustCache: true });
    const newParentCid = asPositiveInt(modified.parentCid);
    if (!settings.isConfigured ||
      !settings.effectiveCategoryIds.includes(cid) ||
      !newParentCid ||
      !settings.effectiveCategoryIds.includes(newParentCid)) {
      // Namespace moved outside the wiki (or to top level): leave the page be.
      return;
    }

    const wikiTombstones = require("./wiki-tombstones");
    const tids = [...new Set((await Promise.all(
      settings.effectiveCategoryIds.map((eachCid) => db.getSortedSetRange(`cid:${eachCid}:tids`, 0, -1))
    )).flat().map(asPositiveInt).filter(Boolean))];
    const rows = (await topics.getTopicsFields(tids, [
      "tid", "cid", "uid", "deleted", "scheduled", "westgateWikiNamespaceIndexCid",
      ...wikiTombstones.TOMBSTONE_FIELDS
    ])) || [];
    const candidates = rows.filter((row) =>
      row &&
      asPositiveInt(row.westgateWikiNamespaceIndexCid) === cid &&
      asPositiveInt(row.cid) !== newParentCid &&
      !parseInt(row.deleted, 10) &&
      !parseInt(row.scheduled, 10) &&
      !wikiTombstones.isTombstonedTopic(row));
    if (candidates.length !== 1) {
      // Zero: no stored index page. More than one: stale duplicates — moving
      // the wrong one is worse than leaving both; skip and let an admin sort it.
      return;
    }

    const indexTopic = candidates[0];
    await wikiTopicMutations.withTopicMutationGuard(indexTopic.tid, async () => {
      await wikiTopicMutations.withManagedMutationContext(() => topics.tools.move(
        indexTopic.tid,
        wikiTopicMutations.markManagedMutation({ cid: newParentCid, uid: asPositiveInt(indexTopic.uid) || 1 })
      ));
    });
  } catch (err) {
    // Never let index-page relocation break the category update itself.
    console.error(`[westgate-wiki] failed to relocate namespace index page for cid=${cid}: ${err && err.stack || err}`);
  }
}
...
module.exports = {
  ...
  relocateNamespaceIndexPageOnCategoryMove,
  saveWikiPage
};
