...
function createApplyServices(options = {}) {
  ...
  const articleCss = require("../content/wiki-article-css");
  const discussionSettings = require("../read/wiki-discussion-settings");
  ...
  return {
    ...
    pages: {
      ...
      async updatePage(payload) {
        ...
        const wikiPageActions = require("../pages/wiki-page-actions");
        const wikiEditLocks = require("../pages/wiki-edit-locks");
        let moved = null;
    ...
    revisions: {
      async ensureCreateRevision(payload) {
        const tid = asPositiveInt(payload && payload.tid);
        const pid = asPositiveInt(payload && (payload.pid || payload.mainPid));
        const cid = asPositiveInt(payload && payload.cid);
        const uid = asPositiveInt(payload && payload.uid);
        const source = String(payload && (payload.source || payload.sourceContent || payload.content) || "");
        if (!tid || !pid || !cid || !uid) {
          throw new Error("archive-apply-create-revision-invalid");
        }
        if (!source.trim() || await wikiRevisions.hasRevisions(tid)) {
          return null;
        }
        return await wikiRevisions.appendRevision({
          tid,
          pid,
          cid,
          uid,
          ...
          oldSource: "",
          newSource: source,
          canonicalPath: String(payload && payload.canonicalPath || ""),
          wikiPath: String(payload && payload.wikiPath || "")
        });
      }
    },
    identity: {
