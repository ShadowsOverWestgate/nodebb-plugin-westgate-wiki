...
function createArchiveJobService(options = {}) {
  ...
  const jobs = new Map();
  let ensuredPrivateDir = null;

  function setNow(nextNow) {
    now = typeof nextNow === "function" ? nextNow : now;
  }

  // The default private dir is a fixed, world-readable path under the OS temp
  // dir; on a shared host a local attacker could pre-create it (or symlink it)
  // and capture exported wiki content. Own it with 0700 before writing anything
  // and refuse if we can't (chmod on a dir we don't own throws).
  function ensurePrivateDir() {
    if (!ensuredPrivateDir) {
      ensuredPrivateDir = (async () => {
        await fs.mkdir(privateDir, { recursive: true, mode: 0o700 });
        const stat = await fs.lstat(privateDir);
        if (stat.isSymbolicLink() || !stat.isDirectory()) {
          throw new Error("archive-private-dir-unsafe");
        }
        await fs.chmod(privateDir, 0o700);
      })().catch((err) => {
        ensuredPrivateDir = null;
        throw err;
      });
    }
    return ensuredPrivateDir;
  }

  async function createJob(type, input = {}) {
    // Opportunistic sweep instead of a background timer: bounds both the in-memory
    // Map and the on-disk private dir every time work arrives.
    await cleanupExpiredJobs().catch(() => {});
  ...
  async function completeJob(jobId, artifact = {}) {
    ...
    if (shouldWriteArtifact) {
      ...
      await fs.writeFile(artifactPath, content, { mode: 0o600 });
  ...
  async function cleanupExpiredJobs() {
    ...
    // After a restart the Map is empty but job directories may linger on disk;
    // remove any untracked dir older than the longest retention window so the
    // full-wiki export/import payloads don't orphan forever.
    const maxTtl = Math.max(policy.retention.failedJobTtlMs, policy.retention.completedJobTtlMs);
    try {
      ...
      for (const entry of entries) {
        ...
        const dirPath = path.join(privateDir, entry.name);
        const stat = await fs.stat(dirPath).catch(() => null);
        if (stat && currentTime - stat.mtimeMs >= maxTtl) {
          await fs.rm(dirPath, { recursive: true, force: true });
          removedJobIds.push(entry.name);
        }
      }
    } catch (err) {
      // privateDir may not exist yet — nothing to sweep.
    }
