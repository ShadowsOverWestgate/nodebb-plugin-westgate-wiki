...
async function saveCreatedPage(services, payload) {
  ...
  const saved = normalizePageMutationResult(created, payload);
  const ensureCreateRevision = resolveService(
    services,
    "revisions.ensureCreateRevision",
    "archive-apply-create-revision-service-unavailable"
  );
  await ensureCreateRevision({
    ...payload,
    tid: saved.tid,
    pid: saved.pid,
    mainPid: saved.pid,
    cid: saved.cid,
    source: payload.sourceContent || payload.content || ""
  });
  return saved;
}

async function saveUpdatedPage(services, payload) {
  const updatePage = resolveService(services, "pages.updatePage", "archive-apply-page-update-service-unavailable");
