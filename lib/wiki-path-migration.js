...
const RESERVED_FIRST_SEGMENTS_TEXT = [...RESERVED_FIRST_SEGMENTS].join(", ");
...
function buildBlockingDetails(report = {}) {
  ...
  return [
    ...
    makeBlockingDetail(
      ...
      `Rename the listed categories or move them below a route root so public wiki paths do not start with reserved segments (${RESERVED_FIRST_SEGMENTS_TEXT}).`,
      reservedRoots.namespaces
    ),
    makeBlockingDetail(
      "reserved-page-root",
      "Reserved page route roots",
      `Rename the listed topics or move them below a namespace so public wiki paths do not start with reserved segments (${RESERVED_FIRST_SEGMENTS_TEXT}).`,
      reservedRoots.pages
    ),
    makeBlockingDetail(
